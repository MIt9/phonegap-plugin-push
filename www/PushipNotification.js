//Puship Library V1.4.1 - (c)2015 SoftRay
var Puship = Puship || {};
var Puship = function () {
};
var _pp;
...
Puship.prototype.GCM = function () {
    ...
    return {
        Register: function (GCMProject, optionalparams) {
            ...
            privateInstanceIdentifier = _pp.Common.DefaultValue(optionalparams.instanceId, _pp.Common.GetUuid());
    ...
    return {
        ...
        UnRegister: function (successCallback, failureCallback) {
            ...
            if (pushNotification != null) {
                ...
                pushNotification.unregister(UnregisterSuccessCallback, UnregisterFailCallback, {"channelName": channelName});
            } else {
                _pp.Common.Log("Calling native unregister");
                return cordova.exec(UnregisterSuccessCallback, UnregisterFailCallback, 'GCMPlugin', 'unregister', [{}]);
            }
        ...
        Register: function (optionalparams) {
            ...
            if (pushNotification != null) {
                ...
                pushNotification.register(function (resultToken) {
                    ...
                    _pp.Common.Log('- Are you using a new device that is not present in the provisiong profile?');
                ...
                _pp.APNS.registerDevice({alert: true, badge: true, sound: true}, function (status) {
            ...
            if (window.plugins.pushNotification != null)
                ...
                _pp.Common.NotifyPush(ConvertPush(notification.aps));
        }, UnRegister: function (successCallback, failureCallback) {
...
Puship.prototype.Common = function () {
    ...
    function AddTagFilter(tag, optionalparams) {
        ...
        var getstr = _pp.Domain + "services/puship.svc/AddTagFilter?AppId='" + appKey + "'&DeviceIdentifier='" + encodeURIComponent(deviceId)
    ...
    function RemoveTagFilter(tag, optionalparams) {
        ...
        var deviceId = appKey.toString() + "_" + privateInstanceIdentifier;
        _pp.Common.Log('DeviceIdentifier:' + deviceId);
        var getstr = _pp.Domain + "services/puship.svc/RemoveTagFilter?AppId='" + appKey + "'&DeviceIdentifier='" + encodeURIComponent(deviceId)
            + "'&Tag='" + tag + "'&$format=json";
        _pp.Common.Log('RequestTo:' + getstr);
        request.open('GET', getstr, true);
        request.onreadystatechange = function () {
    ...
    function UnRegisterFromPuship(optionalparams) {
        ...
        var deviceId = appKey.toString() + "_" + privateInstanceIdentifier;
        _pp.Common.Log('DeviceIdentifier:' + deviceId);
        var getstr = _pp.Domain + "services/puship.svc/UnRegisterDevice?AppId='" + appKey + "'&DeviceIdentifier='" + encodeURIComponent(deviceId)
            + "'&$format=json";
        _pp.Common.Log('RequestTo:' + getstr);
        request.open('GET', getstr, true);
        request.onreadystatechange = function () {
    ...
    function DeletePushMessage(push_id, optionalparams) {
        ...
        var deviceId = appKey.toString() + "_" + privateInstanceIdentifier;
        _pp.Common.Log('DeviceIdentifier:' + deviceId);
        var getstr = _pp.Domain + "services/puship.svc/DeletePushMessages?AppId='" + appKey + "'&strPushMessageId='" + push_id
            + "'&$format=json";
        _pp.Common.Log('RequestTo:' + getstr);
        request.open('GET', getstr, true);
        request.onreadystatechange = function () {
    ...
    function CleanTagFilter(optionalparams) {
        ...
        var deviceId = appKey.toString() + "_" + privateInstanceIdentifier;
        _pp.Common.Log('DeviceIdentifier:' + deviceId);
        var getstr = _pp.Domain + "services/puship.svc/CleanTagFilter?AppId='" + appKey + "'&DeviceIdentifier='" + encodeURIComponent(deviceId)
            + "'&$format=json";
        _pp.Common.Log('RequestTo:' + getstr);
        request.open('GET', getstr, true);
    ...
    function GetTagFilters(optionalparams) {
        ...
        var deviceId = appKey.toString() + "_" + privateInstanceIdentifier;
        _pp.Common.Log('DeviceIdentifier:' + deviceId);
        var getstr = _pp.Domain + "services/puship.svc/GetTagFilters?AppId='" + appKey + "'&DeviceIdentifier='" + encodeURIComponent(deviceId)
            + "'&$format=json";
        _pp.Common.Log('RequestTo:' + getstr);
        request.open('GET', getstr, true);
        request.onreadystatechange = function () {
    ...
    function pushCurrentPositionSuccess(position) {
        _pp.Common.Log('Latitude: ' + position.coords.latitude + '\n' + 'Longitude: ' + position.coords.longitude + '\n' + 'Altitude: ' + position.coords.altitude + '\n' + 'Accuracy: ' + position.coords.accuracy + '\n' + 'Altitude Accuracy: ' + position.coords.altitudeAccuracy + '\n' + 'Heading: ' + position.coords.heading + '\n' + 'Speed: ' + position.coords.speed + '\n' + 'Timestamp: ' + position.timestamp + '\n');
    ...
    function GetPushMessagesInternal(optionalparams) {
        ...
        var deviceId = appKey.toString() + "_" + privateInstanceIdentifier;
        _pp.Common.Log('DeviceIdentifier:' + deviceId);
        var getstr = _pp.Domain + "Services/Puship.svc/GetPushMessages?AppId='" + appKey + "'&DeviceType=" + GetCurrentOs().value + "&Limit=" + privateLimit + "&Offset=" + privateOffset + "&Tags='" + privateTags + "'" + "&IncludeParams=" + privateIncludeParams + "";
        if (adddevicepush) {
    ...
    function GetPushMessagesByDevice(optionalparams) {
        ...
        var deviceId = appKey.toString() + "_" + privateInstanceIdentifier;
        _pp.Common.Log('DeviceIdentifier:' + deviceId);
        var getstr = _pp.Domain + "Services/Puship.svc/GetPushMessagesByDevice?DeviceId='" + encodeURIComponent(deviceId) + "'&Limit=" + privateLimit + "&Offset=" + privateOffset;
        getstr += "&$format=json";
        _pp.Common.Log('RequestTo:' + getstr);
        request.open('GET', getstr, true);
        request.onreadystatechange = function () {
    ...
    function RegisterOnPuship(deviceToken, appKey, deviceType, optionalparams) {
        ...
        var getstr = _pp.Domain + "services/puship.svc/RegisterDevice?AppId='" + appKey + "'&DeviceType=" + deviceType + "&Token='" + deviceToken + "'&DeviceIdentifier='" + encodeURIComponent(deviceId) + "'&language='en'";
        getstr += '&$format=json';
        _pp.Common.Log('RequestTo:' + getstr);
        request.open('GET', getstr, true);
        request.onreadystatechange = function () {
    ...
    function RegisterCurrentPosition(optionalparams) {
        ...
        if (!optionalparams) {
            ...
            if (positionOptionalParams.callMinutes != null && positionOptionalParams.callMinutes < _pp.MinimumSingleLocationCallDelayMinutes)
                positionOptionalParams.callMinutes = _pp.MinimumSingleLocationCallDelayMinutes;
            if (positionOptionalParams.minimumAccuracy == null)positionOptionalParams.minimumAccuracy = _pp.MinimumAccuracy;
        }
        positionOptionalParams.successCallback = DefaultValue(positionOptionalParams.successCallback, function (pushipresult) {
            _pp.Common.Log('Internal puship RegisterPosition success managed');
        });
        positionOptionalParams.failCallback = DefaultValue(positionOptionalParams.failCallback, function (pushipresult) {
        ...
        if (lastpositionCheck != null) {
            var now = new Date();
            if (lastpositionCheck.getTime() + positionOptionalParams.callMinutes * 60000 >= now.getTime()) {
    ...
    function verifyAccuracy(accuracyEnabled, currentAccuracy, minAccuracy) {
        if (accuracyEnabled == true && currentAccuracy != null && minAccuracy != null) {
            if (currentAccuracy < minAccuracy)return true; else {
    ...
    function WatchPositionV2(optionalparams) {
        ...
        if (!WatchPositionIsActiveV2) {
            ...
            if (!optionalparams) {
                ...
                if (positionWatchOptionalParamsV2.callMinutes != null && positionWatchOptionalParamsV2.callMinutes < _pp.MinimumLocationCallDelayMinutes)positionWatchOptionalParamsV2.callMinutes = _pp.MinimumLocationCallDelayMinutes;
                if (positionWatchOptionalParamsV2.minimumAccuracy == null)positionWatchOptionalParamsV2.minimumAccuracy = _pp.MinimumAccuracy;
            }
            if (GetCurrentOs() == _pp.OS.ANDROID) {
            ...
            watchTimeoutIdV2 = setInterval(function () {
                ...
                if (!WatchPositionIsActiveV2 && watchTimeoutIdV2 != null && countItemOnLocalStorage("watchPositionQueue") == 0) {
            ...
            return navigator.geolocation.watchPosition(watchPositionSuccessV2, currentWatchPositionError, positionWatchOptionalParamsV2);
        }
    }

    function watchPositionSuccessV2(position) {
        if (!verifyAccuracy(positionWatchOptionalParamsV2.enableHighAccuracy, position.coords.accuracy, positionWatchOptionalParamsV2.minimumAccuracy)) {
            _pp.Common.Log('Position skipped due to inadeguate accuracy');
            return;
        }
        _pp.Common.Log('New pos, Lat:' + position.coords.latitude + ", Lon:" + position.coords.longitude);
        pushItemToLocalStorage("watchPositionQueue", convertPosition(position));
    }

    function internalWatchTryToUploadV2() {
        var positiontoupload = getLastItemsFromLocalStorage("watchPositionQueue", _pp.PositionQueueLenght);
        ...
        var privateInstanceIdentifier = DefaultValue(positionWatchOptionalParamsV2.instanceId, _pp.Common.GetUuid());
        var pSuccessCallBack = DefaultValue(positionWatchOptionalParamsV2.successCallback, function (pushipresult) {
            _pp.Common.Log('Internal puship RegisterPosition success managed');
        });
        var pFailCallBack = DefaultValue(positionWatchOptionalParamsV2.failCallback, function (pushipresult) {
            _pp.Common.Log('Internal puship RegisterPosition fail managed');
        });
        var request = new XMLHttpRequest();
        var deviceId = positionAppKey.toString() + "_" + privateInstanceIdentifier;
        var getstr = _pp.Domain + "services/puship.svc/RegisterMultiPosition?AppId='" + positionAppKey + "'&DeviceIdentifier='" + encodeURIComponent(deviceId) + "'&$format=json";
        request.open('POST', getstr, true);
        request.setRequestHeader("Content-type", "application/x-www-form-urlencoded; charset=UTF-8");
        request.setRequestHeader("Content-length", postparams.length);
        request.setRequestHeader("Connection", "close");
    ...
    function currentPositionSuccess(position) {
        if (!verifyAccuracy(positionOptionalParams.enableHighAccuracy, position.coords.accuracy, positionOptionalParams.minimumAccuracy)) {
        ...
        var privateInstanceIdentifier = DefaultValue(positionOptionalParams.instanceId, _pp.Common.GetUuid());
        _pp.Common.Log('Registering Position...');
        var request = new XMLHttpRequest();
        _pp.Common.Log('AppId:' + positionAppKey);
        var deviceId = positionAppKey.toString() + "_" + privateInstanceIdentifier;
        _pp.Common.Log('DeviceIdentifier:' + deviceId);
        var getstr = _pp.Domain + "services/puship.svc/RegisterPosition?AppId='" + positionAppKey + "'&DeviceIdentifier='" + encodeURIComponent(deviceId)
    ...
    function Log(l) {
        ...
        if (_pp.EnableHtmlLog == true) {
            ...
            if (htmllog) {
                if (htmllog.innerHTML.length > 5000)htmllog.innerHTML = htmllog.innerHTML.substring(0, 5000);
                htmllog.innerHTML = new Date().toLocaleTimeString() + " - " + l + "<br>" + htmllog.innerHTML;
            }
        }
    }
...
if (!window.plugins.puship) {
    ...
    _pp.Domain = 'http://puship.cloudapp.net/';
    _pp.gcmregid = '';
}
